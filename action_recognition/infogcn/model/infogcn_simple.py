...
class InfoGCN(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, encoding_channels=64, in_channels=3,
                 drop_out=0, num_head=3, noise_ratio=0.1, k=0, gain=1):
        super(InfoGCN, self).__init__()

        A = np.stack([np.eye(num_point)] * num_head, axis=0)

        base_channel = encoding_channels
        ...
        self.l3 = EncodingBlock(base_channel, base_channel*2, A, stride=2)
        self.l4 = EncodingBlock(base_channel*2, base_channel*2, A)
        self.l5 = EncodingBlock(base_channel*2, base_channel*4, A, stride=2)
        self.l6 = EncodingBlock(base_channel*4, base_channel*4, A)
    ...
    def forward(self, x):
        ...
        A = self.A_vector.to(device=x.device, dtype=x.dtype).expand(N*M*T, -1, -1)
        x = A @ x
