...
from adult_afar_multi import (
    adult_afar_multi,
    set_models_dir as set_adult_models_dir,
    set_max_num_faces as set_adult_max_num_faces,
)
from infant_afar_multi import (
    infant_afar_multi,
    set_models_dir as set_infant_models_dir,
    set_max_num_faces as set_infant_max_num_faces,
)
...
def build_arg_parser():
    ...
    p.add_argument("--max-num-faces", type=int, default=5, help="Maximum faces to detect per frame")
...
def main():
    ...
    set_adult_max_num_faces(args.max_num_faces)
    set_infant_max_num_faces(args.max_num_faces)

    if args.mode == "infant":
        aus = parse_list(args.aus, INFANT_OCC)
        data = infant_afar_multi(args.input, aus, bool(args.gpu), args.max_frames, fill_value=args.fill_value)
    else:
        aus = parse_list(args.aus, ADULT_OCC)
        au_int = parse_list(args.au_int, ADULT_INT)
        data = adult_afar_multi(
            args.input,
            aus,
            bool(args.gpu),
            args.max_frames,
