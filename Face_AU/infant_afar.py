...
def crop_img(image):
    ...
    if results.multi_face_landmarks:
        ...
        return imgs[0], all_lands
        
    else:
        return None


def infant_afar(filename, AUs, GPU,  max_frames, fill_value=0):
    _resolve_models_dir()
    print(filename)
    ...
    while cap.isOpened():
        ...
        try:
            ...
            ft, lands = crop_img(frame)
            if ft is None or lands is None:
    ...
    to_pd = {}
    to_pd["Frame"] = flist[:]              # all frames, including missing-face frames
    n = len(flist)
...
if __name__ == "__main__":
    ...
    fg = infant_afar("videos/FN035_FF_B_AV_473.avi", ["au_6"], True, 1000)
    for k in fg:
       print(k)
       print(len(fg[k]))
